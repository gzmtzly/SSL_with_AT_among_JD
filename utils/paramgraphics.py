...
def tile_raster_images(X, img_shape, tile_shape, tile_spacing=(0, 0),
    ...
    if colorImg:
        ...
        channelSize = int(X.shape[1] / 3)  # X.shape = (4000, 3, 32, 32)
    ...
    out_shape = [int(i) for i in out_shape]
    ...
    if isinstance(X, tuple):
        ...
        if output_pixel_vals:
            out_array = np.zeros((int(out_shape[0]), int(out_shape[1]), 4), dtype='uint8')
        else:
            out_array = np.zeros((int(out_shape[0]), int(out_shape[1]), 4), dtype=X.dtype)
...
# Matrix to image
def mat_to_img(w, dim_input, scale=False, colorImg=False, tile_spacing=(1,1), tile_shape=0, save_path=None):
    if tile_shape == 0:
